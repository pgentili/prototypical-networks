...
IMAGENET_DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../data/miniImagenet')
IMAGENET_CACHE = { }
...
def convert_tensor(key, d):
    d[key] = torch.from_numpy(np.array(d[key], np.float32, copy=False) / 255.).permute(2, 0, 1)
    return d
...
def load_class_images(d):
    if d['class'] not in IMAGENET_CACHE:
        ...
        if len(class_images) == 0:
            raise Exception("No images found for miniImagenet class {} at {}.".format(d['class'], image_dir))
...
def load(opt, splits):
    split_dir = os.path.join(IMAGENET_DATA_DIR, 'splits', opt['data.split'])
